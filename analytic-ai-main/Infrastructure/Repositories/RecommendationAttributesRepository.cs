...
namespace Infrastructure.Repositories
{
    public class RecommendationAttributesRepository(DTADbContext context) : Repository<RecommendationAttributes>(context), IRecommendationAttributesRepository
    {
        ...
        public async Task<IReadOnlyCollection<Guid>> GetBySessionIdsAsync(IEnumerable<Guid> sessionIds)
        {
            var sessionIdsHashSet = sessionIds.ToHashSet();
            return await Entities
                .Where(x => sessionIdsHashSet.Contains(x.SessionId))
